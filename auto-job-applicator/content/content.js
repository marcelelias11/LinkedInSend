...
(function() {
  ...
  function showFormDetectedNotification() {
    ...
    document.getElementById('fill-submit-btn').addEventListener('click', function() {
      fillAndSubmitForm();
      hideNotification();
    });
  ...
  function fillAndSubmitForm() {
    chrome.storage.sync.get(['profileData', 'appSettings'], function(data) {
      if (!data.profileData) {
        console.error('No profile data found');
        return;
      }
      ...
      // Check if one-click submission is enabled
      const allowSubmit = !data.appSettings || data.appSettings.oneClickSubmit !== false;
      
      if (allowSubmit && formInfo.submitButton) {
        ...
        try {
          ...
          formInfo.submitButton.dispatchEvent(clickEvent);
        ...
        alert('One-click submission is disabled in settings');
      } else {
        alert('Unable to find the submit button for this form');
      }
    });
  }
  ...
  function getJobTitle() {
    ...
    const selectors = [
      'h1.job-title', 'h1.jobtitle', 'h1.posting-title', 'h1.position-title',
      '.job-title', '.jobtitle', '.posting-title', 
      'h1:contains("Job Title")', 'h2:contains("Position")'
    ];
  ...
  function getCompanyName() {
    ...
    const selectors = [
      '.company-name', '.company', '.employer-name', '.organization-name',
      'span[itemprop="hiringOrganization"]', '[data-testid="company-name"]'
    ];
    ...
    try {
      ...
      if (domainParts.length > 1 && !['com', 'org', 'net', 'io', 'co'].includes(domainParts[domainParts.length - 2])) {
